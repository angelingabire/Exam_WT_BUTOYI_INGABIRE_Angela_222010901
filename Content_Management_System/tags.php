...
<section>
    ...
    <form method="post" onsubmit="return confirmInsert();">
        ...
        <input type="number" id="tag_id" name="tag_id"><br><br>

        <label for="tag_name">Tag name:</label>
        <input type="name" id="tag_name" name="tag_name"><br><br>
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $stmt = $connection->prepare("INSERT INTO tags(tag_id,tag_name) VALUES (?, ?)");
    $stmt->bind_param("is", $tag_id,$tag_name);
    ...
    <table border="6">
        ...
        if ($result->num_rows > 0) {
            ...
            while ($row = $result->fetch_assoc()) {
                $tid = $row['tag_id']; // Fetch the tag_id
                echo "<tr>
                    <td>" . $row['tag_id'] . "</td>
                    <td>" . $row['tag_name'] . "</td>               
                    <td><a style='padding:4px' href='delete_tags.php?tag_id=$tid'>Delete</a></td> 
                    <td><a style='padding:4px' href='update_tags.php?tag_id=$tid'>Update</a></td> 
                </tr>";
            }
        } else {

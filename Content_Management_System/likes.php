...
<section>
    ...
    <form method="post" onsubmit="return confirmInsert();">
        ...
        <label for="liked_at">Liked at:</label>
        <input type="date" id="liked_at" name="liked_at" required><br><br>
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $stmt = $connection->prepare("INSERT INTO likes(like_id, article_id,user_id,liked_at) VALUES (?, ?, ?,?)");
    $stmt->bind_param("iiis", $like_id, $article_id, $user_id,$liked_at);
    ...
    <table border="5">
        ...
        if ($result->num_rows > 0) {
            ...
            while ($row = $result->fetch_assoc()) {
                $lkid = $row['like_id']; // Fetch the like_id
                echo "<tr>
                    <td>" . $row['like_id'] . "</td>
                    <td>" . $row['article_id'] . "</td>
                    <td>" . $row['user_id'] . "</td>
                    <td>" . $row['liked_at'] . "</td>
                    <td><a style='padding:4px' href='delete_likes.php?like_id=$lkid'>Delete</a></td> 
                    <td><a style='padding:4px' href='update_likes.php?like_id=$lkid'>Update</a></td> 
                </tr>";
            }
        } else {

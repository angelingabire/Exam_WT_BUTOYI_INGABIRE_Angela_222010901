...
<section>
    ...
    <h1><u> User roles Form </u></h1>
    <form method="post" onsubmit="return confirmInsert();">
            
        <label for="user_role_id">User role id:</label>
        <input type="number" id="user_role_id" name="user_role_id"><br><br>
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $stmt = $connection->prepare("INSERT INTO user_roles(user_role_id,user_id,role_id) VALUES (?, ?, ?)");
    $stmt->bind_param("iii", $user_role_id, $user_id,$role_id);
...
// SQL query to fetch data from the comments table
...
<head>
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <title>Detail information Of User roles</title>
    <style>
        table {
            width: 100%;
...
<body>
    <center><h2>Table of User roles</h2></center>
    <table border="6">
        <tr>
            <th>user role id</th>
            <th>user id</th>
        ...
        if ($result->num_rows > 0) {
            ...
            while ($row = $result->fetch_assoc()) {
                $user_role_id = $row['user_role_id']; // Fetch the user_role_id
                echo "<tr>
                    <td>" . $row['user_role_id'] . "</td>
                    <td>" . $row['user_id'] . "</td>
                    <td>" . $row['role_id'] . "</td>
                    <td><a style='padding:4px' href='delete_user_roles.php?user_role_id=$user_role_id'>Delete</a></td> 
                    <td><a style='padding:4px' href='update_user_roles.php?user_role_id=$user_role_id'>Update</a></td> 
                </tr>";
            }
        } else {
